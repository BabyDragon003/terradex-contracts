...
use classic_terrapexc::asset::{AssetInfo, PairInfo};
...
use cosmwasm_std::{
    attr, coin, from_binary, to_binary, Addr, ContractResult, CosmosMsg, OwnedDeps, Reply, ReplyOn,
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_terrapexc_factory(&[], &[("uusd".to_string(), 6u8)]);
    let asset_infos = [
...
fn create_pair_native_token_and_ibc_token() {
    ...
    deps = init(deps);
    deps.querier.with_terrapexc_factory(
        &[],
...
fn reply_test() {
    ...
    // register terrapexc pair querier
    deps.querier.with_terrapexc_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {

    ...
    Ok(Response::new().add_submessage(SubMsg {
        ...
        msg: WasmMsg::Instantiate {
            ...
            msg: to_binary(&TokenInstantiateMsg {
                name: "terrapexc liquidity token".to_string(),
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateConfig { owner, treasury } => {
            execute_update_config(deps, env, info, owner, treasury)
        }
...
pub fn swap(
    ...
    if !commission_amount.is_zero() {
        let treasury_rate = Decimal256::from_str(TREASURY_RATE).unwrap();
        let commission_origion_amount: Uint256 = commission_amount.into();
        let amount_commission: Uint256 =
            (Decimal256::from_uint256(commission_origion_amount)) * Uint256::one();
        let treasury_amount: Uint256 = amount_commission * treasury_rate;
        let treasury_asset = Asset {
            info: ask_pool.info.clone(),
            amount: treasury_amount.into(),
        };
        let config = CONFIG.load(deps.storage)?;
        messages.push(treasury_asset.into_msg(&deps.querier, config.treasury.clone())?);
    }
...
/// we compute new spread else we just use terrapexc

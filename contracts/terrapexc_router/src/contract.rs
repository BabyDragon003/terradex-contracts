...
use classic_terrapexc::pair::{QueryMsg as PairQueryMsg, SimulationResponse};
use classic_terrapexc::querier::{query_pair_info, reverse_simulate};
use classic_terrapexc::router::{
...
const CONTRACT_NAME: &str = "crates.io:terrapexc-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            terrapexc_factory: deps.api.addr_canonicalize(&msg.terrapexc_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        terrapexc_factory: deps
            .api
            .addr_humanize(&state.terrapexc_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    for operation in operations.into_iter() {
        ...
        offer_amount = match operation {
            ...
            SwapOperation::TerraPexc {
                offer_asset_info,
                ask_asset_info,
            } => {
                let terrapexc_factory = deps.api.addr_humanize(&config.terrapexc_factory)?;

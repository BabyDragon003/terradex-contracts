...
pub fn execute_order(
    ...
    if let Some(update_order) = update_order {
        match_orders.push(update_orderbook(
            deps.storage,
            deps.api,
            deps.querier,
            &order,
    ...
    if let Some(remove_orders) = remove_orders {
        for remove_order in remove_orders.iter() {
            match_orders.push(remove_orderbook(
                deps.storage,
                deps.api,
                deps.querier,
                &order,
    ...
    for match_order in match_orders.iter() {
        ...
        let other_move_amount = move_amount * order.price / Uint128::from(NORMAL_DECIMAL);
...
pub fn update_orderbook(
    ...
    let pair_info = cfg
        .pair_list
        .get(update_order.pair_id.u128() as usize)
        .unwrap();
    ...
    if update_order.is_buy {
        ...
        buyer_record.current_stock_amount = buyer_record.current_stock_amount - move_amount;
...
pub fn remove_orderbook(
    ...
    let pair_info = cfg
        .pair_list
        .get(remove_order.pair_id.u128() as usize)
        .unwrap();
